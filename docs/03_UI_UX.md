...
- 홈 히어로의 첫 번째 CTA는 `사진 번역하기`와 "색약·색맹인이 사진 속 색의 차이를 더 구분하기 쉽게, 새 사진을 만들어요."를 함께 보여 준다. 보조 CTA는 `시야 확인하기`와 "색약·색맹이 없는 사람이 색약·색맹인의 시야를 시뮬레이션으로 확인해요. 원본은 바꾸지 않아요."를 함께 보여 두 기능의 결과가 다름을 분명히 한다. Translate 내부 탭은 `내가 전할 장면`과 `그 사람에게 전해질 모습`으로 구분한다.
