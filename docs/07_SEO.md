...
```
/translate (EN)
  title: Colorblind-Friendly Photo Converter — Show Them the Beauty | Iris
...
/simulate (EN)
  title: Color Blindness Simulator — See Through Their Eyes | Iris
...
/learn/deuteranopia (KO)
  title: 적록색약(Deutan)이 보는 세상 — 사진으로 비교해보기 | Iris
...
- [ ] Learn/Home 완전 SSG, 에디터 페이지도 셸은 정적 HTML (크롤러가 콘텐츠를 봄)
- [ ] `hreflang`: en ↔ ko 상호 링크 + `x-default` = en (next-intl + metadata API)
- [ ] `sitemap.xml` / `robots.txt` 빌드 타임 생성
- [ ] Canonical: locale별 self-canonical
- [ ] Structured Data:
  - `WebApplication` (Home) — free, browser-based
  - `FAQPage` (`/learn/faq`)
