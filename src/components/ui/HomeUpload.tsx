...
export function HomeUpload({ locale, label, error }: { locale: "en" | "ko"; label: string; error: string }) {
  ...
  const openImage = (file?: File) => {
    ...
    reader.onload = () => {
      try {
        sessionStorage.setItem("iris:pending-image", String(reader.result));
        router.push("/translate", { locale });
      } catch {
        setMessage(error);
      }
    };
