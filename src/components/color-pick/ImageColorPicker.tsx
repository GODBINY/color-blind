...
export function ImageColorPicker({ locale }: { locale: string }) {
  ...
  function saveImage() {
    ...
    picks.forEach((pick, index) => {
      ...
      context.fillStyle = "#2D2330"; context.font = "600 14px system-ui"; context.fillText(`${pick.id}. ${describeColor(pick.rgb, locale)}`, left + 48, top + 14);
      context.fillStyle = "#2D2330"; context.font = "600 14px ui-monospace, monospace"; context.fillText(hexOf(pick.rgb), left + 48, top + 33);
      context.fillStyle = "#6E6474"; context.font = "12px ui-monospace, monospace"; context.fillText(`RGB ${pick.rgb.join(", ")}`, left + 48, top + 49);
    });
  ...
  function removePick(id: number) {
    setPicks((current) => current.filter((pick) => pick.id !== id).map((pick, index) => ({ ...pick, id: index + 1 })));
  }
