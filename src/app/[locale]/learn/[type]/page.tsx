...
import { getLearnContent, getLocalizedLearnName, isLearnLocale, learnLocales, learnTypes, type LearnLocale, type LearnType } from "@/lib/learn/content";
import { LEARN_ARTICLE_DATES, siteUrl } from "@/lib/seo";
import type { Metadata } from "next";

const updatedLabels: Record<LearnLocale, string> = {
  ko: "업데이트",
  en: "Updated",
};
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; type: string }> }): Promise<Metadata> {
  ...
  if (!isLearnLocale(locale)) {
    return {
      robots: { index: false, follow: true },
      alternates: { canonical: `/en/learn/${type}` },
    };
  }
  const item = getLearnContent(locale, type as LearnType);
  const path = `/${locale}/learn/${type}`;
  const languages = Object.fromEntries(learnLocales.map((itemLocale) => [itemLocale, `/${itemLocale}/learn/${type}`]));
  return {
    ...
    openGraph: {
      ...
      title: item.title,
      description: item.intro,
      publishedTime: LEARN_ARTICLE_DATES.published,
      modifiedTime: LEARN_ARTICLE_DATES.modified,
      authors: [siteUrl().toString()],
    },
  };
}
...
export default async function LearnTypePage({
  ...
  if (!isLearnLocale(locale)) permanentRedirect(`/en/learn/${type}`);
  setRequestLocale(locale);
  const item = getLearnContent(locale, type as LearnType);
  const localizedName = getLocalizedLearnName(locale, type as LearnType);
  const isKo = locale === "ko";
  const formattedModifiedDate = new Intl.DateTimeFormat(locale, {
    year: "numeric",
    month: "long",
    day: "numeric",
    timeZone: "Asia/Seoul",
  }).format(new Date(LEARN_ARTICLE_DATES.modified));
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "Article",
    headline: item.title,
    description: item.intro,
    image: new URL("/images/iris-hero-flower.jpg", siteUrl()).toString(),
    datePublished: LEARN_ARTICLE_DATES.published,
    dateModified: LEARN_ARTICLE_DATES.modified,
    author: { "@type": "Organization", name: "NUNBIT", url: siteUrl().toString() },
    inLanguage: locale,
