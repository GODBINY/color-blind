    ...
    <html lang={locale} suppressHydrationWarning>
      ...
      <body>
        ...
        <NextIntlClientProvider>
          <header className="mx-auto flex max-w-[1184px] items-center justify-between border-b border-[var(--color-border)] px-4 py-1 md:px-8">
            <div className="flex min-w-0 items-center gap-1">
              <HeaderBackButton locale={locale} />
              <HeaderBrandLink label={t("brand")} />
            </div>
            <nav aria-label={t("languageNav")} className="flex items-center gap-2 text-[11px] font-medium text-[var(--color-text-sub)]">
              <details className="relative">
                <summary className="flex min-h-8 cursor-pointer list-none items-center gap-1 rounded-[var(--radius-s)] px-1.5 hover:bg-[var(--color-bg)] focus-visible:outline-3 focus-visible:outline-[var(--color-accent)]"><span aria-hidden="true">◎</span>{languages.find(([code]) => code === locale)?.[1] ?? "English"}<span aria-hidden="true">⌄</span></summary>
